...
(function () {
  ...
  function renderPageSearch() {
    return "<wc-products></wc-products>";
  }
  ...
  function renderPageCart() {
    return "<wc-listcart></wc-listcart>";
  }

  function render(page) {
    if (page.includes("search")) {
      $app.innerHTML = renderPageSearch();
    } else if (page.includes("product")) {
      var path = page.split("/");
      var product_id = path[path.length - 1];
      $app.innerHTML = renderPageProduct(product_id);
    } else if (page.includes("cart")) {
      $app.innerHTML = renderPageCart();
    }
  }
